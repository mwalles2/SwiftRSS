...
class RSSParser_Tests: XCTestCase {
	...
	let PDTTimeZone: TimeZone! = TimeZone(identifier: "PST")
	let GMTTimeZone: TimeZone! = TimeZone(identifier: "GMT")
	let DSTTimeZone: TimeZone! = TimeZone(secondsFromGMT: 60 * 60 * -4)
	...
	func test_parser_withValidMock_shouldReturnTheRightValues() {
		...
		RSSParser.parseFeedForRequest(request, callback: { (feed, error) -> Void in
			...
			self.calendar.timeZone = self.PDTTimeZone
			...
			if let testFeed = feed
			{
				...
				{
					var dateComponent = self.calendar.dateComponents(self.calendarFlags, from: date)
					...
					XCTAssert(dateComponent.timeZone == self.PDTTimeZone, "")
				}
				else
				{
	...
	func test_parser_withWordpressMock_shouldReturnTheRightValues() {
		...
		RSSParser.parseFeedForRequest(request, callback: { (feed, error) -> Void in
			...
			self.calendar.timeZone = self.GMTTimeZone
			...
			if let testFeed = feed
			{
				...
				if let date = testFeed.lastBuildDate
				{
					...
					XCTAssert(dateComponent.timeZone! == self.GMTTimeZone, "")
				}
				else
				{
					XCTFail("lastBuildDate shouldn't be nil")
				...
				if (myItem.imagesFromItemDescription.count == 1) {
				...
				if(myItem.imagesFromContent.count == 7) {
	...
	func test_parser_withTumblrMock_shouldReturnTheRightValues() {
		...
		RSSParser.parseFeedForRequest(request, callback: { (feed, error) -> Void in
			...
			self.calendar.timeZone = self.DSTTimeZone
			...
			if let testFeed = feed
			{
				...
				if let date = myItem.pubDate
				{
					...
					XCTAssert(dateComponent.timeZone == self.DSTTimeZone, "")
				}
				else
				{
					XCTFail("pubDate shouldn't be nil")
